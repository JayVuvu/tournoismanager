class TournamentManager {
    ...
    saveMatchScore(matchId) {
        ...
        this.tournament.matches.forEach((round, roundIndex) => {
            round.forEach(match => {
                if (match.id === matchId) {
                    matchFound = true;
                    const score1Input = document.querySelector(`input[data-match="${matchId}"][data-team="1"]`);
                    const score2Input = document.querySelector(`input[data-match="${matchId}"][data-team="2"]`);
    ...
    updateTeamStats(match) {
        ...
        if (!scores[match.team1.id]) scores[match.team1.id] = { points: 0, goalsFor: 0, goalsAgainst: 0 };
        if (!scores[match.team2.id]) scores[match.team2.id] = { points: 0, goalsFor: 0, goalsAgainst: 0 };
    ...
    organizeMatchesInRounds(matches, courts) {
        ...
        for (let i = 0; i < matches.length; i++) {
            ...
            if (currentRound.length === courts || i === matches.length - 1) {
